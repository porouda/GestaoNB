...
export const ChecklistPage = ({ user }: { user?: any }) => {
  ...
  useEffect(() => {
    const unsubPhases = onSnapshot(query(collection(db, 'checklist_phases'), orderBy('order')), (snap) => {
    ...
    const unsubPrograms = onSnapshot(query(collection(db, 'checklist_programs'), orderBy('name')), (snap) => {
